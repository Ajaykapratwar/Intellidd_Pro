...
Write a comprehensive, professional due diligence report in Markdown.
Use sector-specific language, metrics, and benchmarks appropriate for a {sector_label} company.
Where data exists, cite specific numbers and compare to sector benchmarks.

IMPORTANT: If "UPLOADED DOCUMENT CONTEXT" is present above, cross-reference it with the
web research. Where the documents provide more detail or contradict web findings, note this
explicitly in the relevant section. Cite document sources as [From uploaded document].
...
## Executive Summary
(4-5 paragraphs: what the company does, key findings, sector positioning, overall investment signal.
If documents were uploaded, note what they revealed beyond public information.)
...
If financial documents were uploaded, cross-reference with public signals.)
...
Key differentiators and competitive risks from competitor analysis.)

## Risk Assessment
(Top 5 risks with severity — prioritize risks most impactful in {sector_label}:
...
(Top 5 strengths — evaluated against what matters in {sector_label})
...
*Document sources: [list any uploaded documents used, or "None uploaded"]*

Write the full report now. Be specific — cite actual data points.
Compare to sector benchmarks. Do not make up numbers not in the research data.
"""
...
{{
  "competitors": [
    {{
      ...
      "website": "string — competitor URL",
      "description": "string — one sentence on what they do",
      "founded_year": "string or 'Unknown'",
      "headquarters": "string or 'Unknown'",
      "funding_total": "string — e.g. '$50M' or 'Unknown'",
      "funding_stage": "string — Seed / Series A / etc or 'Unknown'",
      "employee_estimate": "string — e.g. '50-200' or 'Unknown'",
      "key_differentiator": "string — what makes them different",
      "overlap_score": "integer 1-10 — how directly do they compete with {company_name}? (10 = direct substitute)",
      "strengths_vs_target": ["list of ways this competitor is stronger than {company_name}"],
      "weaknesses_vs_target": ["list of ways this competitor is weaker than {company_name}"]
    }}
  ],
  "comparison_matrix": {{
    "dimensions": ["Funding", "Team Size", "Product Maturity", "Market Presence", "Tech Depth", "Community/Brand"],
    "scores": {{
      "{company_name}": [score1, score2, score3, score4, score5, score6],
      "competitor_1_name": [score1, score2, score3, score4, score5, score6],
      "competitor_2_name": [score1, score2, score3, score4, score5, score6]
    }}
  }},
  "market_position": "string — Leader / Strong Challenger / Challenger / Niche Player / New Entrant",
  "differentiation_score": "integer 1-100 — how differentiated is {company_name} from competitors?",
  "moat_assessment": "string — what is {company_name}'s defensible competitive advantage?",
  "biggest_competitive_threat": "string — which competitor poses the most risk and why?",
  "market_consolidation_signal": "string — is this market consolidating? Any M&A activity?",
...
RISK_SCORER_PROMPT = """
You are a senior investment risk analyst. You have received the full due diligence research package for a company. Your job is to produce a structured, quantitative risk scorecard.
...
Score each of the 5 risk dimensions on a scale of 1-10 where:
  1-3 = LOW RISK (strong signals, well-covered)
  4-6 = MEDIUM RISK (some concerns or gaps)
  7-9 = HIGH RISK (significant red flags)
  10  = CRITICAL RISK (deal-breaker level concern)
...
{{
  "founder_risk": {{
    ...
    "key_factors": ["list of 3-4 specific factors that drove this score"],
    "evidence": "string — specific data points from research supporting this score",
    "mitigation": "string — what would reduce this risk?"
  }},
  "market_risk": {{
    "score": "integer 1-10",
    "severity": "Low / Medium / High / Critical",
    "key_factors": ["list of 3-4 specific factors"],
    "evidence": "string",
    "mitigation": "string"
  }},
  "financial_risk": {{
  ...
  "overall_risk_score": "integer 1-10 — weighted average across all 5 dimensions",
  "dd_confidence_score": "integer 1-100 — how confident are we in this assessment? (100 = full data, 0 = almost no data)",
  "highest_priority_risk": "string — which single risk dimension needs most attention?",
  "risk_summary": "string — 3-4 sentence overall risk narrative for this {sector_label} company"
}}
